...
def test_get_currency_rates_api_error() -> None:
    ...
    """
    currencies = ["USD", "EUR", "GBP"]  # Define currencies *inside* the test function
    with patch("requests.get") as mock_get:
        mock_get.side_effect = requests.exceptions.RequestException("API Error")
        result = get_currency_rates(currencies, max_retries=1, retry_delay=0)  # Pass retry parameters
        assert "USD" in result, "USD должен быть в результатах"
        assert "EUR" in result, "EUR должен быть в результатах"
        assert "GBP" in result, "GBP должен быть в результатах"
