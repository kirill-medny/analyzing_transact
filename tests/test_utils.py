...
from src.utils import (get_greeting, get_card_data, get_top_transactions, get_currency_rates, get_stock_data,
                       filter_transactions_by_month, get_date_range, filter_transactions_by_date_range,
                       get_expenses_data, get_income_data, analyze_cashback_categories, investment_bank, simple_search,
                       search_phone_numbers, search_person_transfers, report_decorator, spending_by_category)
...
def test_get_greeting_with_fixtures(time_fixture, expected_greeting, request):
    """
    ...
    assert greeting == expected_greeting, f"Для времени {current_time} ожидалось приветствие '{expected_greeting}', но было получено '{greeting}'"
...
def test_get_greeting_with_hours(hour, expected_greeting):
    """
    Тест функции get_greeting с параметризацией по часу.
    """
    current_time = time(hour=hour, minute=0, second=0)
    greeting = get_greeting(current_time)
    assert greeting == expected_greeting, f"Для часа {hour} ожидалось приветствие '{expected_greeting}', но было получено '{greeting}'"

@patch("src.utils.time")  # Замените src.utils на фактический путь к модулю
def test_get_greeting_mock_time(mock_time):
    """
...
def test_get_card_data_empty_dataframe():
    """
...
def test_get_card_data_single_card(sample_dataframe):
    """
...
def test_get_card_data_multiple_cards(sample_dataframe):
    """
...
def test_get_card_data_parameterized(sample_dataframe, card_number, expected_total_spent, expected_cashback):
    """
...
def test_get_card_data_mock_unique(mock_unique, sample_dataframe):
    """
...
def test_get_top_transactions_less_than_5_transactions(sample_dataframe_top_transactions):
    """
    Тест, когда в DataFrame меньше 5 транзакций.
    """
    df = sample_dataframe_top_transactions.sort_values(by='Сумма платежа', ascending=True).head(3)  # Сначала сортируем, потом берем 3 строки
...
def test_get_top_transactions_exactly_5_transactions(sample_dataframe_top_transactions):
    """
...
def test_get_top_transactions_more_than_5_transactions(sample_dataframe_top_transactions):
    """
...
def test_get_top_transactions_date_format(sample_dataframe_top_transactions):
    """
...
def test_get_currency_rates_success(currencies, mock_response):
    """
...
def test_get_currency_rates_api_error(currencies):
    """
...
def test_get_currency_rates_rub_excluded(currencies):
    """
...
def test_get_currency_rates_parameterized(currencies, mock_response, currency, expected_value):
    """
...
def test_filter_transactions_by_month_empty_dataframe():
    """
    Тест для случая, когда DataFrame пустой.
    """
...
def test_filter_transactions_by_month_single_month(sample_dataframe_by_month):
    """
...
def test_filter_transactions_by_month_same_date_as_transaction(sample_dataframe_by_month):
    """
...
def test_filter_transactions_by_month_date_at_start_of_month(sample_dataframe_by_month):
    """
...
def test_filter_transactions_by_month_date_at_end_of_month(sample_dataframe_by_month):
    """
...
def test_get_date_range_invalid_range():
    """
...
def test_get_date_range_week_start_of_week():
    """
...
def test_get_date_range_week_end_of_week():
    """
...
def test_filter_transactions_by_date_range_empty_dataframe():
    """
    Тест для случая, когда DataFrame пустой.
    """
    empty_df = pd.DataFrame({'Дата операции': [], 'Сумма платежа': [], 'Категория': []})
...
def test_filter_transactions_by_date_range_within_range(sample_dataframe_by_month):
    """
...
def test_filter_transactions_by_date_range_end_date_at_end(sample_dataframe_by_month):
    """
...
def test_filter_transactions_by_date_range_range_covers_multiple_months(sample_dataframe_by_month):
    """
    ...
    assert (result['Дата операции'] >= start_date).all(), "Все даты должны быть больше или равны start_date"
    assert (result['Дата операции'] <= end_date).all(), "Все даты должны быть меньше или равны end_date"
    assert (result['Дата операции'].dt.month <= 2).all(), "Все месяцы должны быть январь или февраль"  # Дополнительная проверка
def test_filter_transactions_by_date_range_no_matches(sample_dataframe_by_month):
    """
...
def test_filter_transactions_by_date_range_start_date_greater_than_end_date(sample_dataframe_by_month):
    """
...
def test_get_expenses_data_empty_dataframe():
    """
    Тест для случая, когда DataFrame пустой.
    """
    empty_df = pd.DataFrame({'Дата операции': [], 'Сумма платежа': [], 'Категория': []})
...
def test_get_expenses_data_basic(sample_expenses_dataframe):
    """
...
def test_get_expenses_data_only_positive_amounts(sample_expenses_dataframe):
    """
...
def test_get_expenses_data_transfers_and_cash_categories(sample_expenses_dataframe):
    """
...
def test_get_expenses_data_top_categories_limit(sample_expenses_dataframe):
    """
    ...
    data = {'Дата операции': [], 'Сумма платежа': [], 'Категория': []}
    for i in range(10):
...
def test_get_expenses_data_no_transfers_or_cash(sample_expenses_dataframe):
    """
...
def test_get_expenses_data_same_category_name(sample_expenses_dataframe):
    """
...
def test_get_income_data_empty_dataframe():
    """
    Тест для случая, когда DataFrame пустой.
    """
    empty_df = pd.DataFrame({'Дата операции': [], 'Сумма платежа': [], 'Категория': []})
...
def test_get_income_data_basic(sample_income_dataframe):
    """
...
def test_get_income_data_only_negative_amounts(sample_income_dataframe):
    """
...
def test_get_income_data_single_category(sample_income_dataframe):
    """
...
def test_get_income_data_zero_amounts(sample_income_dataframe):
    """
...
def test_get_income_data_empty_categories(sample_income_dataframe):
    """
...
def test_analyze_cashback_categories_basic(sample_cashback_dataframe):
    """
...
def test_analyze_cashback_categories_no_data(sample_cashback_dataframe):
    """
...
def test_analyze_cashback_categories_only_positive_amounts(sample_cashback_dataframe):
    """
    ...
    expected = {}  # Должен возвращать пустой словарь
    assert json.loads(result) == expected, "Должен возвращать пустой словарь, если нет расходов"


def test_analyze_cashback_categories_single_category(sample_cashback_dataframe):
    """
...
def test_analyze_cashback_categories_multiple_months(sample_cashback_dataframe):
    """
...
def test_analyze_cashback_categories_exception_handling(mock_logging, sample_cashback_dataframe):
    """
...
def test_analyze_cashback_categories_no_expenses(sample_cashback_dataframe):
    """
...
def test_analyze_cashback_categories_rounding(sample_cashback_dataframe):
    """
...
def test_investment_bank_no_transactions_for_month():
    """
...
def test_investment_bank_empty_transactions_list():
    """
...
def test_investment_bank_limit_is_one():
    """
...
def test_investment_bank_transactions_with_same_date_different_amounts():
    """
...
def test_investment_bank_transactions_with_amounts_multiple_of_limit():
    """
...
def test_investment_bank_exception_handling(mock_logging):
    """
    Тест для проверки обработки исключений.
    """
    transactions = [
    ...
    assert mock_logging.exception.called, "Должно быть записано в лог об исключении" # Проверяем, что был вызов exception

# Тесты для simple_search

def test_simple_search_basic(sample_search_dataframe):
    """
    ...
    assert all(item['Описание'] in ['Покупка в магазине ABC', 'Покупка в магазине DEF'] for item in result_list), "Описание должно содержать 'магазин'"

def test_simple_search_case_insensitive(sample_search_dataframe):
    """
...
def test_simple_search_in_category(sample_search_dataframe):
    """
...
def test_simple_search_no_results(sample_search_dataframe):
    """
...
def test_simple_search_empty_search_string(sample_search_dataframe):
    """
...
@patch('src.utils.logging')
def test_simple_search_exception_handling(mock_logging, sample_search_dataframe):
    """
    Тест для проверки обработки исключений.
    """
...
def test_search_phone_numbers_basic(sample_phone_dataframe):
    """
    ...
    assert all(item['Описание'] in ['Оплата по номеру +7 (912) 345-67-89',
                                   'Перевод другу 8 (903) 123 45 67',
                                   'Покупка в магазине, номер телефона +1234567890'] for item in result_list), "Описание должно содержать телефонный номер"

def test_search_phone_numbers_no_results(sample_phone_dataframe):
    """
...
def test_search_phone_numbers_different_formats(sample_phone_dataframe):
    """
...
def test_search_phone_numbers_empty_dataframe():
    """
    Тест для случая, когда DataFrame пустой.
    """
    empty_df = pd.DataFrame({
...
@patch('src.utils.logging')
def test_search_phone_numbers_exception_handling(mock_logging, sample_phone_dataframe):
    """
    Тест для проверки обработки исключений.
    """
...
def test_search_person_transfers_basic(sample_person_transfers_dataframe):
    """
    ...
    assert all(item['Описание'] in ['Перевод Иванову И.',
                                   'Перевод Петрову А.',
                                   'Перевод Сидорову Б.',
                                   'Перевод Орлову Г.'] for item in result_list), "Описание должно соответствовать переводу физлицу"

def test_search_person_transfers_no_results(sample_person_transfers_dataframe):
    """
...
def test_search_person_transfers_wrong_category(sample_person_transfers_dataframe):
    """
...
def test_search_person_transfers_empty_dataframe():
    """
    Тест для случая, когда DataFrame пустой.
    """
    empty_df = pd.DataFrame({
...
def test_search_person_transfers_invalid_name_format(sample_person_transfers_dataframe):
    """
...
@patch('src.utils.logging')
def test_search_person_transfers_exception_handling(mock_logging, sample_person_transfers_dataframe):
    """
    Тест для проверки обработки исключений.
    """
    # Создаем DataFrame, где столбец 'Описание' имеет некорректный тип
